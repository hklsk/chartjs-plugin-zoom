...
function panTimeScale(scale, delta, panOptions) {
  var options = scale.options;
  const min = rangeMinLimiter(panOptions, scale.getValueForPixel(scale.getPixelForValue(scale.min) - delta));
  const max = rangeMaxLimiter(panOptions, scale.getValueForPixel(scale.getPixelForValue(scale.max) - delta));
  if (min != options.time.min && max != options.time.max) {
